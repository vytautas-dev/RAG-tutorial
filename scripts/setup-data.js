...
async function setupData() {
    ...
    try {
        ...
        try {
            const collectionInfo = await qdrantClient.getCollection(config.qdrantCollection);
        ...
        if (shouldLoadData) {
            ...
            logger.info('Start embedding documents... This may take a few minutes on the first run');
            ...
            logger.success('All documents have been embedded and loaded into the vector store');
        }

        logger.info('5: Verifying data in the database');

        // Verification
        const finalCollectionInfo = await qdrantClient.getCollection(config.qdrantCollection);
        ...
        if (error.message.includes('ECONNREFUSED') || error.message.includes('fetch failed')) {
            logger.error('❌ Unable to connect to Qdrant. Please check if the Qdrant server is running and accessible.');
        }
        process.exit(1);
    }
}
